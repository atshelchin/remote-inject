...
describe('request() local methods', () => {
  ...
  it('web3_clientVersion returns RemoteInject', async () => {
    const { provider } = setup();
    expect(await provider.request({ method: 'web3_clientVersion' })).toBe('RemoteInject/1.0.0');
  });
  ...
  it('eth_accounts returns copy of accounts when connected', async () => {
    ...
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'connect', data: undefined });
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'accountsChanged', data: ['0xabc'] });
...
describe('handleMessage wp-event', () => {
  it('accountsChanged updates state and emits', () => {
    ...
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'accountsChanged', data: ['0xnew'] });
  ...
  it('chainChanged updates chainId and networkVersion', () => {
    ...
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'chainChanged', data: '0x89' });
  ...
  it('chainChanged handles numeric data', () => {
    const { provider, handleMessage } = setup();
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'chainChanged', data: 56 });
  ...
  it('disconnect clears state', () => {
    ...
    // Connect first
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'connect', data: undefined });
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'accountsChanged', data: ['0xa'] });
    expect(provider.isConnected()).toBe(true);

    // Disconnect
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'disconnect', data: undefined });
  ...
  it('message event is forwarded', () => {
    ...
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'message', data: { type: 'eth_subscription', data: {} } });
...
describe('handleMessage wp-init-state', () => {
  ...
  it('ignores init with no accounts', () => {
    const { provider, handleMessage } = setup();
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-init-state', connected: true, accounts: [], chainId: '0x1' });
    expect(provider.isConnected()).toBe(false);
  });
});
...
describe('event listener API', () => {
  ...
  it('once() fires handler only once', () => {
    ...
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'chainChanged', data: '0x5' });
    handleMessage({ channel: 'remote-inject-ext', type: 'wp-event', event: 'chainChanged', data: '0x6' });
...
describe('PROVIDER_INFO', () => {
  it('has correct EIP-6963 fields', () => {
    expect(PROVIDER_INFO.uuid).toBe('f81d4fae-7dec-11d0-a765-00a0c91e6bf6');
    expect(PROVIDER_INFO.name).toBe('Remote Inject');
    expect(PROVIDER_INFO.rdns).toBe('com.remote-inject.bridge');
