<script lang="ts">
  ...
  function relativeTime(ts: number): string {
    ...
    if (diff < 5) return t('now', '刚刚');
    if (diff < 60) return t(`${diff}s ago`, `${diff} 秒前`);
    if (diff < 3600) { const m = Math.floor(diff / 60); return t(`${m}m ago`, `${m} 分钟前`); }
    const h = Math.floor(diff / 3600);
    return t(`${h}h ago`, `${h} 小时前`);
  }
  ...
  function categoryLabel(c: string): string {
    switch (c) {
      case 'sign': return t('Sign', '签名');
      case 'tx': return t('Tx', '交易');
      case 'auth': return t('Auth', '授权');
      case 'read': return t('Read', '读取');
      default: return c;
    }
  }
  ...
  function statusLabel(status: ActivityEntry['status']): string {
    switch (status) {
      case 'pending': return t('Awaiting wallet', '等待钱包');
      case 'success': return t('Completed', '已完成');
      case 'rejected': return t('Rejected', '已拒绝');
      case 'error': return t('Failed', '失败');
    }
  }
</script>
  ...
  <div class="activity">
    <div class="activity-header">
      ...
      {#if onClear && entries.length > 0}
        <button class="clear-btn" onclick={onClear} title={t('Clear activity log', '清除活动记录')}>
    ...
    {#if filteredEntries.length === 0}
      <div class="empty-filter">{t('No activity from this site yet', '该网站暂无活动记录')}</div>
    {:else}
            ...
            <div class="detail-panel" id={`activity-detail-${entry.id}`}>
              <div class="detail-grid">
                <div class="detail-field">
                  <span class="detail-label">{t('Method', '方法')}</span>
              ...
              <div class="detail-field">
                <span class="detail-label">{t('Origin', '来源')}</span>
                <code class="detail-value">{entry.origin}</code>
              </div>
              <div class="detail-field">
                <span class="detail-label">{t('Params', '参数')}</span>
                {#if entry.params !== undefined}
                  <pre class="detail-pre">{formatJson(entry.params)}</pre>
                {:else}
                  <span class="detail-empty">{t('No parameters', '无参数')}</span>
                {/if}
              </div>
              {#if entry.status === 'pending'}
                <div class="pending-note">
                  <span class="pending-dot"></span>
                  {t('The request is waiting for a response from the wallet.', '该请求正在等待钱包响应。')}
                </div>
              {/if}
              {#if entry.result !== undefined}
                <div class="detail-field">
                  <span class="detail-label">{t('Result', '结果')}</span>
                  <pre class="detail-pre result">{formatJson(entry.result)}</pre>
                </div>
              {/if}
              {#if entry.error}
                <div class="detail-field">
                  <span class="detail-label">{t('Error', '错误')}</span>
