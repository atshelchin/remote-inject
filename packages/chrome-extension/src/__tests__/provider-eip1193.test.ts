...
describe('Request routing logic', () => {
  ...
  it('web3_clientVersion returns WalletPair version string', async () => {
    ...
    expect(result).toBe('RemoteInject/1.0.0');
  });
...
describe('EIP-6963 announcement', () => {
  ...
  it('info has correct uuid', () => {
    expect(info.uuid).toBe('f81d4fae-7dec-11d0-a765-00a0c91e6bf6');
  });

  it('info has correct name', () => {
    expect(info.name).toBe('Remote Inject');
  });
  ...
  it('rdns is com.remote-inject.bridge', () => {
    expect(info.rdns).toBe('com.remote-inject.bridge');
  });
