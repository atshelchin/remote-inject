...
export default defineContentScript({
  ...
  main() {
    ...
    const { provider, handleMessage } = createProvider(
      ...
      () => window.dispatchEvent(new CustomEvent('remote-inject:open-panel')),
    );
    ...
    // Always set window.remoteInject for direct access
    Object.defineProperty(window, 'remoteInject', {
      value: provider,
      writable: false,
      configurable: false,
    });
