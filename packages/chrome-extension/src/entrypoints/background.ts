...
function scheduleReconnect() {
  ...
  reconnectAttempt++;
  console.warn(`[RemoteInject] Scheduling reconnect in ${delay}ms (attempt ${reconnectAttempt})`);
  reconnectTimer = setTimeout(async () => {
    ...
    if (ok && reconnectAllowed) {
      reconnectAttempt = 0;
      console.log('[RemoteInject] Reconnected successfully');
...
function updateState(patch: Partial<ExtensionState>) {
  ...
  chrome.runtime.sendMessage({ action: 'state-update', state }).catch((e) => console.warn('[RemoteInject]', e));
}
  ...
  const onPhase = (phase: string) => {
    switch (phase) {
      ...
      case 'connected':
        ...
        saveSessionState(sessionRef.serialize()).catch((e) => console.warn('[RemoteInject]', e));
        saveConnectedAt(Date.now()).catch((e) => console.warn('[RemoteInject]', e));
        chrome.alarms.clear('remote-inject-keepalive', () => {
          chrome.alarms.create('remote-inject-keepalive', { periodInMinutes: 0.5 });
        });
      ...
      case 'disconnected':
        ...
        break;
      case 'closed':
        void handleSessionClosed().catch((e) => console.warn('[RemoteInject]', e));
        break;
    }
  };
...
function handleEthereumEvent(sessionRef: WalletPairSession, event: EthereumEvent) {
  if (event.event === 'accountsChanged') {
    ...
    if (accounts.length > 0) {
      ...
      saveConnectedWallet(connectedWallet).catch((e) => console.warn('[RemoteInject]', e));
      ...
      return new Promise((resolve) => {
        addDeferredRequest(id, payload, (response) => {
          if (origin && !response.error) {
            grantPermission(origin).catch((e) => console.warn('[RemoteInject]', e));
          }
          if (shouldLog) {
  ...
  try {
    ...
    if (method === 'eth_requestAccounts') {
      ...
      if (accounts.length > 0) {
        ...
        saveConnectedWallet(connectedWallet).catch((e) => console.warn('[RemoteInject]', e));
        updateState({ wallet: { ...connectedWallet } });
        if (origin) grantPermission(origin).catch((e) => console.warn('[RemoteInject]', e));
      }
    }
...
export default defineBackground(() => {
  ...
  chrome.runtime.onMessage.addListener((msg: BackgroundMessage, sender: any) => {
    ...
    (chrome.sidePanel as any)
      .open({ tabId })
      .catch((e: any) => console.warn('[RemoteInject] open-panel failed:', e));
  ...
  chrome.runtime.onMessage.addListener((msg: BackgroundMessage, sender, sendResponse) => {
    (async () => { try {
      ...
      console.error('[RemoteInject] Unhandled error in message handler:', err);
  ...
  chrome.alarms.onAlarm.addListener(async (alarm) => {
    if (alarm.name !== 'remote-inject-keepalive') return;
    ...
    if (!saved) return;
    const ok = await tryReconnect();
    if (ok) console.log('[RemoteInject] reconnected via keepalive backstop');
  });

  // On SW start, try to restore session
  tryReconnect().then((ok) => {
    if (ok) {
      console.log('[RemoteInject] Session restored after SW wake');
